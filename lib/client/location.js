...
var popped = false;
var previousUrl;
var ignoreRoutesForPreviousUrl;
...
IronLocation.getPreviousUrl = function () {
  dep.depend();
  return previousUrl;
}

Reload._onMigrate('previousUrl', function(retry) {
  var data = { previousUrl: previousUrl }
  return [true, data];
});

Meteor.startup(function() {
  var data = Reload._migrationData('previousUrl') || {};
  previousUrl = data.previousUrl || '';
});

IronLocation.config = function(options) {
  ignoreRoutesForPreviousUrl = options.ignoreRoutesForPreviousUrl || [];
}
...
IronLocation.pushState = function (state, title, url) {
  ...
  if (!_.contains(ignoreRoutesForPreviousUrl, currentRoute))
