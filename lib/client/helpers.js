if (Handlebars) {
  ...
  /*
   * Take a nav name and returns a class if the nav is currently
   * selected. The class defaults to 'selected'.
   */
  Handlebars.registerHelper('navClassFor', function (name, cls) {
    if (arguments.length == 2) {
      cls = 'selected';
    }

    return _.contains(Router.getNav(), name) && cls;
  });
