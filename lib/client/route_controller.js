...
RouteController = Utils.extend(IronRouteController, {
  constructor: function () {
    ...
    // Get nav values from inherited controllers.
    var collectInheritedNav = function(ctor) {
      var navs = [];

      if (ctor.__super__)
        navs = navs.concat(collectInheritedNav(ctor.__super__.constructor));

      return Utils.toArray(ctor.prototype.nav).concat(navs);
    }

    /*
     * Add the routes name, all inherited controllers, this controller's
     * and the route's nav values.
     */
    this.nav = [self.route.name]
      .concat(Utils.toArray(getOption('nav')))
      .concat(collectInheritedNav(this.constructor));
  ...
  /*
   * A list of nav values that we can check against to decide if
   * we should display a nav element as selected. Should always contain
   * the route name. Apart from that it might contain user specified values.
   */
  nav: [],

  getNav: function () {
    return this.router && this.router.getNav();
  },

  setNav: function (nav) {
    this.router && this.router.setNav(nav);
  },
  ...
  run: function () {
    ...
    // Step 2: If we're not stopped, set the nav and run the action
    ...
    // Step 4: Run the after hooks that we only want to run once.
    if (!this.afterOnceHasRun) {
      this.runHooks('afterOnce', [
        autoClearUnusedYieldsHook
      ]);
      this.afterOnceHasRun = true;
    }
